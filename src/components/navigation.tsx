    ...
    <nav className="grid min-h-screen md:grid-cols-[220px_1fr] lg:grid-cols-[280px_1fr] fixed">
      <div className="border-r bg-muted/40 block">
        <div className="flex h-full max-h-screen flex-col p-4 gap-2">
          <Link
            className="flex flex-row gap-2 font-bold text-2xl border-b border-spacing-10 pb-2"
            href="/"
          >
          ...
          {selectedServer ? (
            <>
              <Select
                onValueChange={(v) => router.push(`/server/${v}`)}
                value={selectValue}
              >
              ...
              <div className="grid items-start text-sm font-medium lg:pre-4">
                {navItems.map(({ href, Icon, label }) => (
                  <NavLink key={href} href={pathname + href} className="w-full">
            ...
            <>
              <h1 className="text-lg font-bold text-primary">Servers</h1>
              {servers.map((server) => (
                <Link
                  key={server.serverId}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Card
                  ...
                  onMouseLeave={() =>
                    setDisplay(session.user?.username as string)
                  }
                >
